...
const FlagAlert: FC<Props> = ({ flag, thumb, alert }): JSX.Element => {
  const keyWordsGreen = flag?.green.keywords.join(", ");
  const keyWordsRed = flag?.red.keywords.join(", ");
  // const { t } = useTranslation("candidateChecker");

  const thumbUpKeywords = useMemo(() => {
    const words = thumb.up.keywords.map((item) => {
      if (!validateKeywords.includes(item)) {
        return "";
      }
      return `thumb_up_${item}`;
    });
  ...
  const thumbDownKeywords = useMemo(() => {
    const words = thumb.down.keywords.map((item) => {
      console.log(item, "item");
      if (!validateKeywords.includes(item)) {
        return "";
      }
      return `thumb_down_${item}`;
    });
  ...
  return (
    <div className="flex gap-2 justify-between" id="flag-container">
      <div className="flex gap-2 flex-col w-full">
        <div className="flex items-center gap-2 flex-col justify-start w-full">
          <div className="flex items-center gap-2 w-full">
            <div className="flex items-start gap-2 h-full">
              <div className="flex items-center gap-2 w-[40px] justify-between">
                {/* Like */}
                <svg
                  width="24"
                  height="24"
            ...
            <div className="flex-1">
              <p className="font-body5" style={{ wordBreak: "break-word" }}>
                {thumbUpKeywords}
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex gap-2 items-center">
            <div className="min-w-6">
              <div className="w-[20px] h-[20px] bg-[var(--green-6)] rounded-full"></div>
            </div>
            <div className="font-h10 text-[var(--green-6)]">
              {flag?.green?.count}
            </div>
            <p className="font-body5 text-[var(--black-85)]">{keyWordsGreen}</p>
          </div>

          <div className="flex gap-2 items-center">
            <div className="min-w-6">
              <div className="w-[20px] h-[20px] bg-[var(--error)] rounded-full"></div>
            </div>
            <div className="font-h10 text-[var(--error)]">
              {flag?.red?.count}
            </div>
            <p className="font-body5 text-[var(--black-85)]">{keyWordsRed}</p>
          </div>
